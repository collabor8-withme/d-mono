...
function YARNorNPMAdapter(dependencies, sourceId, config, level = 1, processedDeps = new Set()) {
    for (const depName in dependencies) {
        ...
        const nestedPkgJson = path__namespace.join(NODE_MODULES_DIR, depName, "package.json");
        const content = fs__namespace.readFileSync(nestedPkgJson, {
            encoding: "utf-8"
        });
...
function coreHook(config) {
    ...
    if (PKG_MANAGER === "yarn" || PKG_MANAGER === "npm") {
        const content = fs__namespace.readFileSync(PKG_JSON_DIR, {
            encoding: "utf-8"
        });
